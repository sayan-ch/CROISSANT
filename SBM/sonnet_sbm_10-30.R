...
write_csv(data.table(`simulation` = 0, `model` = NULL, n = 0, K = 0,
                     `cand.K` = 0, B = "",
                     ...
                     `auc.only.dcbm` = "",
                     `auc.both` = ""), 
          "final_sbm/sonnet_sbm_10-30.csv", append = F)
...
for(i in 1:100){
  ...
  for(par in seq_along(sonpar)){
    ...
    ), "final_sbm/sonnet_sbm_10-30.csv", append = T)
    ...
    write_csv(data.table(simulation = i, model = "SBM",n = nn, K = KK,
               ...
               s = ss, o = oo, R = RR,
               out.sonn$loss),
              "tmp_sbm/sonnet_sbm_loss_10-30.csv", append = T)
